...
def run_telegram_bot():
    """Запускает тестовый Telegram бота (test_bot4.py) в отдельном процессе."""
    global bot_process
    
    try:
        logger.info("Запуск тестового Telegram бота в отдельном процессе...")
        ...
        # Запускаем тестовый бот (test_bot4.py) в отдельном процессе с перенаправлением вывода
        ...
        print(f"[INFO] Тестовый Telegram бот запущен с PID: {bot_process.pid}")
        logger.info(f"Тестовый Telegram бот запущен с PID: {bot_process.pid}, логи в {bot_log_file}")
        
        return True
        
    except Exception as e:
        logger.error(f"Ошибка при запуске тестового бота: {e}")
        print(f"[ОШИБКА] Не удалось запустить тестовый Telegram бота: {e}")
        return False
...
async def main():
    ...
    try:
        ...
        # Запускаем тестовый бот (test_bot4.py)
        if not run_telegram_bot():
            logger.error("Не удалось запустить тестовый Telegram бота")
            return
        
        # Даем время для инициализации бота
        await asyncio.sleep(5)
        
        # Запускаем отслеживание контрактов Solana
